...
export const InstitutionStudents = () => {
  ...
  const handleRegisterStudent = async (e: React.FormEvent) => {
    ...
    if (!regName || !regEmail || !regStudentId) {
      toast('Please provide Name, Email, and Student ID', 'error');
      return;
    }
  ...
  const getStudentStats = (studentId: string) => {
    const sAttempts = attempts.filter((a) => a.studentId === studentId && (a.status === 'submitted' || a.status === 'auto_submitted'));
    const testCount = sAttempts.length;
    const avgPercentage = testCount > 0
      ? sAttempts.reduce((sum, a) => sum + (a.percentage || 0), 0) / testCount
      : 0;
    const totalViolations = sAttempts.reduce((sum, a) => sum + (a.violations || 0), 0);
  ...
  const handleOpenPerformance = async (student: StudentProfile) => {
    ...
    try {
      ...
      const mappedAttempts = sAttempts.map((a) => {
        ...
        return {
          ...
          status: a.status,
          rawTest: test || { id: a.testId, title: 'Assessment', totalMarks: a.score || 100 }
        };
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Institution Student Directory</h1>
          <p className="text-slate-500">
            Enrolled candidates, batch statistics, test results, integrity metrics, and candidate registration.
          </p>
        </div>
        <Button onClick={() => setIsRegisterOpen(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white">
      ...
      {isRegisterOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="max-w-md w-full border shadow-2xl animate-in">
            ...
            <form onSubmit={handleRegisterStudent}>
              ...
              <div className="p-4 border-t bg-slate-50 flex justify-end gap-3">
                ...
                <Button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white" isLoading={isSubmittingReg}>
                  Create Student
                </Button>
              </div>
            </form>
          ...
          <Card className="max-w-3xl w-full max-h-[85vh] overflow-hidden flex flex-col border shadow-2xl animate-in">
            <div className="p-6 border-b flex items-start justify-between bg-slate-50">
              <div className="flex gap-4">
                <div className="h-12 w-12 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center font-bold text-lg uppercase shrink-0">
      ...
      {/* Proctoring Audit & Trust Score Modal for Institution Admins */}
